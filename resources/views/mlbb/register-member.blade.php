...
<div class="flex items-center space-x-4 p-4 border border-gray-600 rounded-lg bg-gray-700">
    <img src="/logos/{{ basename($team->team_logo) }}" alt="{{ $team->team_name }}" class="w-16 h-16 rounded-full">
    <div>
        <span class="text-green-400 text-xs font-bold uppercase tracking-wider mb-1 block">Selected Team</span>
        <h4 class="text-white font-bold text-xl">{{ $team->team_name }}</h4>
        <p class="text-gray-300 text-sm">Captain: {{ $team->captain->full_name ?? 'N/A' }}</p>
    </div>
    <input type="hidden" name="team_id" value="{{ $team->id }}" id="locked_team_id">
</div>
...
class="w-full accent-border accent-shadow neon-input-field rounded-lg px-3 py-2"
...
class="w-full accent-border accent-shadow neon-input-field rounded-lg px-3 py-2 appearance-none"
required>
</div>
...
const selectedTeamInput = document.getElementById('locked_team_id');
if(!selectedTeamInput) {
    alert('Invalid team session. Please rescan the QR code.');
    return;
}

document.getElementById('conf_team').innerText = selectedTeamInput.closest('.border').querySelector('h4').innerText;
